...
if (isset($_GET['oidC'])) {
  ...
  set order_status=1,order_date='" . date('Y-m-d H:i:s') . "'
  where order_id = " . $oidC;
  if ($conn->query($upQuery)) {
    ...
    $upQuery = "update tbl_plant 
    set plant_stock=(plant_stock-" . $cartData['cart_quantity'] . ") 
    where plant_id=" . $cartData['plant_id'];
    if ($conn->query($upQuery)) {
      ?>
      <script>
        alert('Confirmed')
        window.location = './ShopNewOrder.php'
      </script>
      <?php
    }
  }
...
if (isset($_GET['oidR'])) {
  $oidC = $_GET['oidR'];
  $upQuery = "update tbl_order set order_status=3 where order_id = " . $oidC;
  if ($conn->query($upQuery)) {
    ?>
    <script>
      alert('Rejected')
      ...
      <div class="container-fluid pt-4 px-4">
        <div class="text-center rounded p-4" style="background: #fff;">
          <div class="d-flex align-items-center justify-content-between mb-4">
            <h6 class="mb-0" style="color: #548302;font-size: 18pt;">New orders</h6>
          </div>
                ...
                if ($result->num_rows) {
                  ...
                  while ($row = $result->fetch_assoc()) {
                    ...
                    <tr>
                      ...
                      <td align="center">
                        <?php
                        if ($row['cart_quantity'] <= $row['plant_stock']) {
                          ?>
                          <a class="btn btn-sm btn-primary"
                            href="./ShopNewOrder.php?oidC=<?php echo $row['order_id'] ?>">Confirm</a><br><br>
                          <?php
                        }
                        ?>
                        <a class="btn btn-sm btn-primary"
                          href="./ShopNewOrder.php?oidR=<?php echo $row['order_id'] ?>">Reject</a>
                      </td>
                    </tr>
                    <?php
