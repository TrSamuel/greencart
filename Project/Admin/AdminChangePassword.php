...
if (isset($_POST['btn_update'])) {
    $admin_current_password = $_POST['txt_admin_current_password'];
    $admin_new_password = $_POST['txt_admin_new_password'];
    $admin_cpassword = $_POST['txt_admin_cpassword'];
    if ($data['admin_password'] != $admin_current_password) {
        ?>
        <script>
            alert('Incorrect current password')
            window.location = './AdminChangePassword.php'
        </script>
        <?php
    } else {
        if ($admin_new_password == $admin_cpassword) {
            $upQuery = "update tbl_admin set admin_password='$admin_new_password' where admin_id=" . $data['admin_id'];
                            ...
                            <div class=" rounded p-4 p-sm-5 my-4 mx-3" style="background:#548302;">
                                <div class="form-floating mb-3">
                                    <input type="password" class="form-control" id="floatingText" placeholder="jhondoe"
                                        style="background: #fff;" name="txt_admin_current_password" required>
                                    <label for="floatingText">Current password</label>
                                </div>
                                <div class="form-floating mb-3">
                                    <input type="password" class="form-control" id="floatingText" placeholder="jhondoe"
                                        style="background: #fff;" name="txt_admin_new_password" required>
                                    <label for="floatingText">New password</label>
                                </div>
                                <div class="form-floating mb-3">
                                    <input type="password" class="form-control" id="floatingText" placeholder="jhondoe"
                                        style="background: #fff;" name="txt_admin_cpassword" required>
                                    <label for="floatingText">Confirm new password</label>
                                </div>
