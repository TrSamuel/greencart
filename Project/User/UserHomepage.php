...
if (isset($_POST['btn_filter_submit'])) {
    if (isset($_POST['txt_plant_name'])) {
        $plantName = $_POST['txt_plant_name'];
        $selQuery = "select * from tbl_plant where plant_name like '$plantName%'";
        $result = $conn->query($selQuery);
        if ($result->num_rows) {
            $plantName = $_POST['txt_plant_name'];
            if (isset($_POST['txt_min_price']) && isset($_POST['txt_max_price'])) {
                $min_price = $_POST['txt_min_price'];
                $max_price = $_POST['txt_max_price'];
                if (isset($_POST['radio_price_sort'])) {
                    $price_sort = $_POST['radio_price_sort'];
                    if (isset($_POST['sel_plant_category'])) {
                        $plant_category = $_POST['sel_plant_category'];
                        if (isset($_POST['sel_shop'])) {
                            $shop = $_POST['sel_shop'];
                            if (isset($_POST['sel_rating'])) {
                                $rating = $_POST['sel_rating'];
                                if ($rating >= 4) {
                            ...
                            where p.plant_name like '$plantName%' 
                            and c.plant_category_name = '$plant_category'
                            and s.shop_name = '$shop' 
                            and p.plant_rating between 4 and 5
                            ...
                            and c.plant_category_name = '$plant_category'
                            and s.shop_name = '$shop' 
                            and p.plant_rating between 3 and 4
                            and p.plant_price between
                            $min_price and $max_price 
                            order by p.plant_price $price_sort";
                                } else if ($rating >= 2) {
                            ...
                            and c.plant_category_name = '$plant_category'
                            and s.shop_name = '$shop' 
                            and p.plant_rating between 2 and 3
                            and p.plant_price between
                            $min_price and $max_price 
                            order by p.plant_price $price_sort";
                                } else if ($rating >= 1) {
                            ...
                            and c.plant_category_name = '$plant_category'
                            and s.shop_name = '$shop' 
                            and p.plant_rating between 1 and 2
                            and p.plant_price between
                            $min_price and $max_price 
                            order by p.plant_price $price_sort";
                                }
