...
if (isset($_POST['btn_feedback_submit'])) {
    ...
    $shop_feedback_content = $_POST['txt_feedback_content'];
    $shop_feedback_face = $_POST['radio_feedback_rating'];
    $shop_feedback_time = date('Y-m-d H:i:s');
    $insQuery = "insert into tbl_feedback(feedback_content,feedback_rating,feedback_time,shop_id) 
values('$shop_feedback_content',$shop_feedback_face,'$shop_feedback_time'," . $data['shop_id'] . ")";

    if ($conn->query($insQuery)) {
        ?>
        <script>
            alert('Thanks for your feedback')
            window.location = './ShopHomePage.php'
        </script>
        <?php
    }
}
