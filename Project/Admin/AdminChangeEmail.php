...
if (isset($_POST['btn_update'])) {
    ...
    if ($admin_password == $data['admin_password']) {
        $selQuery = "select * from tbl_admin where admin_email = '$admin_email'";
        $upQuery = "update tbl_admin set admin_email='$admin_email' where admin_id =" . $data['admin_id'];
        $result = $conn->query($selQuery);
        if (!$result->num_rows) {
            if ($conn->query($upQuery)) {
                ...
                <script>
                    ...
                    window.location = './AdminChangeEmail.php'
                </script>
                <?php
            }
        } else {
                            ...
                            <div class=" rounded p-4 p-sm-5 my-4 mx-3" style="background:#548302;">
                                <div class="form-floating mb-3">
                                    <input type="text" class="form-control" id="floatingText" placeholder="jhondoe"
                                        style="background: #fff;" name="txt_admin_email"
                                        value="<?php echo $data['admin_email'] ?>" required>
                                    <label for="floatingText">Admin email</label>
                                </div>
                                <div class="form-floating mb-3">
